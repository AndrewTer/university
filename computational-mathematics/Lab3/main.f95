program Lab3
    ...
    write(*,*) "=========================================================================="
    IFLAG=1
    write(*,*) "RKF45 с шагом 0,02"
    write(*,*)
    
    do i = 1, 21
      call RKF45(F,NEQN,Y,T,TOUT,RELERR,ABSERR,IFLAG,WORK,IWORK)
      print 111, TOUT, Y(1), Y(2)
      tout = tout + tprint
    end do

    111      FORMAT(' T: ',F7.4,2X,'|| OutVal(1): ',F15.6,2X,'|| OutVal(2): ',F15.6)
    
    write(*,*) "=========================================================================="
    write(*,*) "Метод Рунге-Кутты 4-й степени с шагом 0,01"
    write(*,*)
    ...
    write(*,*) "=========================================================================="
    write(*,*) "Метод Рунге-Кутты 4-й степени с шагом 0,001"
    write(*,*)
...
contains
    ...
    REAL    :: K1(NEQN),K2(NEQN),K3(NEQN),K4(NEQN),YN(NEQN),Ytmp(NEQN),Y(NEQN)
    REAL    :: t=0.0
    integer :: counter=0
      T=0.0
      Y(1) = 0.0
...
157   Ytmp=Y
      ...
      Ytmp = Y +(K1/3)
      call F(t+(h/3.0),Ytmp,YN)
      K2= h*YN
      
      Ytmp = Y -(K1/3) + K2
      call F(t+((2*h)/3.0),Ytmp,YN)
      K3 = h*YN
      ...
      Ytmp = Y + (K4)
      YN = Y +1.0/8.0*(K1+3*K2+3*K3+K4)
      Y =YN
      T=h+T
      counter = counter +1
      
      11 FORMAT(' T: ', F7.4,2X, '|| OutVal(1): ', F15.6,2X, '|| OutVal(2): ', F15.6)
      12 FORMAT(' T: ', F7.4,2X, '|| OutVal(1): ', F15.6,2X, '|| OutVal(2): ', F15.6)
      
      IF ((h.LE.0.001).and. mod(counter,20).eq.0) PRINT 11, T,YN(1),YN(2)
      IF ((h.GE.0.01)) PRINT 12, T,YN(1),YN(2)
